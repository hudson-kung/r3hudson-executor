...
function addScriptMessage(message, type = 'info') {
    const scriptConsole = document.getElementById('scriptConsole');
    ...
    scriptConsole.appendChild(messageLine);
    scriptConsole.scrollTop = scriptConsole.scrollHeight;
    
    // Limit console to 50 messages
    const lines = scriptConsole.querySelectorAll('.console-line');
    if (lines.length > 50) {
        lines[0].remove();
    }
}
...
// Simulate script activity
function simulateScriptActivity() {
    const scripts = [
        { message: "Loading script: fe_kill_all.lua", type: "info" },
        { message: "Script loaded successfully", type: "success" },
        { message: "Executing script...", type: "info" },
        { message: "Script executed - 127 players affected", type: "success" },
        { message: "Injecting into game...", type: "info" },
        { message: "Injection successful", type: "success" },
        { message: "Script completed", type: "info" }
    ];
    
    let index = 0;
    const interval = setInterval(() => {
        if (index < scripts.length) {
            addScriptMessage(scripts[index].message, scripts[index].type);
            index++;
        } else {
...
// Initialize script console simulation
setTimeout(() => {
    simulateScriptActivity();
}, 3000);

// Executor Download Function
function downloadExecutor() {
    // Open executor UI in new window
